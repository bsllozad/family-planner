  ...
  return <main className={`app-page ${child?'child-mode':''}`}>
    ...
    <div className="dashboard"><aside><nav><a className="active"><Home/>Inicio</a><a><CalendarDays/>Calendario</a>{!child&&<><a><Users/>Familia</a><a><Settings/>Configuración</a></>}</nav><div className="privacy-badge"><ShieldCheck/><span><strong>{child?'Modo infantil':'Espacio privado'}</strong>{child?'Acciones de adulto bloqueadas.':'Solo tu familia puede verlo.'}</span></div></aside>
      <section className="content"><span className="eyebrow"><Sparkles/> {child?'Tu aventura':'Centro de mando familiar'}</span><h1>{child?`¡Hola, ${context.profile_name}!`:`Bienvenido, ${context.profile_name}`}</h1><p className="lead">{child?'Este es tu espacio. Solo verás tus propias aventuras.':`Todo listo para organizar a ${context.family_name}.`}</p>
        {child?<ChildMissions/>:<><MissionManager familyId={context.family_id} children={children}/><ProfileManager familyId={context.family_id} onChanged={()=>void loadChildren()}/><button className="secondary fit invite-admin" onClick={()=>setOpenInvite(true)}><Users/>Invitar administrador</button></>}
      </section>
    </div>
