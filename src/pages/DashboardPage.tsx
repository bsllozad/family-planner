...
import { isPinRequiredError, isValidAdultPin } from '../lib/adultPin'
...
export function DashboardPage(){
  ...
  const chooseChild=async(id:string)=>{if(!id)return;setBusy(true);setError('');const {error:e}=await supabase.rpc('activate_child_profile',{p_profile_id:id});setBusy(false);if(e&&isPinRequiredError(e.message)){setPendingChildId(id);setPinPurpose('set');setPin('');setOpenPin(true);return}if(e){setError('No se pudo cambiar de perfil. Inténtalo de nuevo.');return}setPendingChildId(null);await refreshContext()}
  const submitPin=async(e:FormEvent)=>{e.preventDefault();setError('');if(!isValidAdultPin(pin)){setError('El PIN debe tener entre 4 y 6 dígitos.');return}setBusy(true);if(pinPurpose==='set'){const {error:pinError}=await supabase.rpc('set_adult_pin',{p_pin:pin});if(pinError){setBusy(false);setError(pinError.message.toLowerCase().includes('invalid pin format')?'El PIN debe tener entre 4 y 6 dígitos.':'No se pudo guardar el PIN. Inténtalo de nuevo.');return}if(pendingChildId){const {error:activateError}=await supabase.rpc('activate_child_profile',{p_profile_id:pendingChildId});if(activateError){setBusy(false);setError('El PIN se guardó, pero no se pudo cambiar de perfil. Inténtalo de nuevo.');return}}setBusy(false);setOpenPin(false);setPin('');setPendingChildId(null);await refreshContext();return}const {data,error:e2}=await supabase.rpc('unlock_adult_mode',{p_pin:pin});setBusy(false);const result=data?.[0] as {success:boolean;locked_until:string|null}|undefined;if(e2||!result?.success){setError(result?.locked_until?`Demasiados intentos. Inténtalo después de ${new Date(result.locked_until).toLocaleTimeString()}.`:'PIN incorrecto.');return}setOpenPin(false);setPin('');await refreshContext()}
